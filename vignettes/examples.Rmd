...
|:-------------------|:---------------------|:---------------------|
|Intercept           |2.557 (2.346, 2.770)  |2.622 (2.448, 2.791)  |
|Own smoking (W1)    |0.365 (0.337, 0.390)  |0.360 (0.334, 0.386)  |
|Gender              |0.047 (-0.042, 0.133) |0.059 (-0.025, 0.144) |
|Peers' smoking (W1) |0.042 (0.001, 0.083)  |0.026 (-0.000, 0.052) |
|N                   |4002                  |4002                  |
|DIC                 |16964                 |18700                 |
...
#>  model1       0.0     0.0      NA           8 k_psis > 0.7
#>  model2     -37.1     9.7    1.00
```
...
The replicated densities are far smoother than the observed distribution, which is dominated by a spike at the top of the smoking scale and carries mass at only a handful of values.
That gap is the gaussian likelihood meeting a discrete, heavily skewed outcome, and it is worth knowing.
It does not bear on the aggregation comparison, since all three models share the same response distribution and differ only in the weight function.
...
```
#> Error:
#> ! object 'mod_bml_lstat' not found
```

```
#> Error in `mutate()`:
#> ℹ In argument: `weight = w_mat[cbind(tid, pos)]`.
#> Caused by error:
#> ! object 'w_mat' not found
```

```
#> Error:
#> ! object 'weight_df' not found
```
