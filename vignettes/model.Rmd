...
Throughout this vignette, model components are illustrated using a single running example: **coalition government survival**. Groups ($i$) are governments, members ($k$) are the political parties forming each government's coalition, and nesting units ($j$) are the countries in which those governments serve. The outcome $y_i^{(g)}$ is how long each government survives before terminating. Group-level covariates $\mathbf{x}_i^{(g)}$ describe the government as a whole (e.g., majority status, ideological heterogeneity, economic conditions); nesting-level covariates $\mathbf{x}_j^{(c)}$ describe the country (e.g., electoral system, investiture requirements); member-level covariates $\mathbf{x}_{ik}^{(p)}$ describe each party (e.g., cohesion); and weight covariates $\mathbf{x}_{ik}^{(w)}$ describe what might make one party's contribution matter more than another's (e.g., holding the prime ministership, seat share). Later sections return to this example to show how each model component would be specified for it.
...
**Example:** Continuing the running example, $p[i]$ returns all parties in government $i$, and $n_i$ is coalition size.
...
## 4. The Extended Multiple-Membership Multilevel Model
...
### 4.3 Hierarchical Nesting-Level Effect
...
### 4.4 Aggregated Member-Level Effect
...
## 5. Parameterizable Weight Functions

### 5.1 Conventional MMMM: Fixed Weights
...
### 5.2 Extended MMMM: Parameterizing Weights
...
### 5.3 Testing Alternative Aggregation Mechanisms
...
## 6. Emergent Aggregation Functions

The weights $w_{ik}$ define a within-group *measure* over members; the aggregation function `fn()` chooses which function of that measure (and of the member attributes) enters the group-level equation. `fn("sum")` — the additive weighted mean $A_x = \sum_k w_{ik} x_{ik}$ — is the default and recovers everything above. The *emergent* types read properties of the whole member set:
...
Each block emits a group-level *feature* whose coefficient is an ordinary main-model coefficient (labeled by the feature name, e.g. `V_x`). Blocks stack: combine mean + variance + concentration in one model. Shape parameters (`c`, `kappa`, `p`) are reported in their own "Function parameters" section and take priors via `prior(..., class = "fn")`.

**Who counts vs. how contributions combine.** Parameters that act on *external* member characteristics (importance, size) belong in `w()`; parameters that act on the *effect attribute* (whatever is in `vars()`) belong in `fn()`. Building weights from the attribute being aggregated (`w(~ exp(om * x))` with `vars(x)`) is near-equivalent to `fn("smax")` — the exponentially tilted mean is the derivative of the log-sum-exp — and `bml` warns when a variable appears in both.

### 6.1 User-written reductions: the expression DSL

Beyond the named types, `fn()` accepts a one-sided formula in a restricted expression DSL. `E(e)` is the expectation $\sum_k w_k e_k$ under the normalized weights; member-level quantities are the `vars()` attributes and the reserved symbol `w`; whitelisted operations (`+ - * / ^`, `exp`, `log`, `ilogit`, `pow`) transpile to JAGS. Any other symbol is a free shape parameter (the same one-parameter rule as `w()`).
...
Identification note: shape parameters reach the outcome only through `coefficient x feature` — if the feature's coefficient is near zero they are unidentified, and a free parameter that *multiplies* an attribute inside a nonlinear `E()` is confounded with the coefficient (a warning fires). Treat inner parameters as cutpoints/sharpness, not slopes.

### 6.2 Named blocks and interactions

Every block has a name (auto-generated, e.g. `A_x`, `V_x`, or set via `name =`). Single-feature blocks can be referenced in main-formula interactions:
...
The macro variable must also appear as a main effect, and must be group-constant (a within-group-varying moderator is a member-paired interaction and belongs in `vars(x:z)`). A bare, non-interacted block reference errors — the block's main effect already enters through the block itself.
...
In `bml`, specify `family = cox()`. The optional `intervals` argument controls the number of intervals for the piecewise constant baseline hazard:
...
## 8. Variance Decomposition and Intraclass Correlation
...
### 8.1 Intraclass Correlation Coefficients (ICC)
...
### 9.2 Autoregressive Random Effects

Member random effects may evolve over time rather than remain constant. The autoregressive walk is part of the effects grammar — it is specified inside `re()`, since it is a property of the random effect:
...
With `ar = TRUE`, the effect follows a random walk across the member's successive group participations:
...
**Time-indexed walk.** Passing a (numeric) time variable instead of `TRUE` — `re(1, ar = year)` — orders each member's participations by that variable and scales the step variance by the elapsed time, so that unobserved heterogeneity drifts more over long gaps than short ones:

\begin{equation}
u_{t} \sim N\!\left(u_{t-1},\; \sigma_{u^{(p)}}^2 \cdot \Delta_t / \bar{\Delta}\right) \tag{17a}
\end{equation}

where $\Delta_t$ is the time gap between the member's consecutive participations and $\bar{\Delta}$ the average gap in the data. The normalization by $\bar{\Delta}$ means equal spacing reproduces the `ar = TRUE` model exactly, and $\sigma_{u^{(p)}}$ keeps its interpretation as the innovation standard deviation per average step. Time values must be unique within each member (a zero gap would make the step variance zero).
...
The walk requires an intercept-only random effect (`re(1, ar = ...)`; it does not combine with random slopes or `cor = TRUE`).

### 9.3 Complementary Membership Groups

Some applications distinguish between two mutually exclusive subsets of potential members whose effects on the group outcome may differ — e.g., a government's coalition parties vs. opposition parties, a student's classroom peers vs. other students in the school, or a firm's alliance partners vs. non-partner competitors. Separate `mm()` blocks can model each subset's distinct contribution:

\begin{equation}
\theta_i^{(p)} = \sum_{k \in p[i]} w_{ik}^{(A)} \left( \mathbf{\beta}^{(A)} \cdot \mathbf{x}_{ik}^{(A)} + u_k^{(A)} \right) + \sum_{k \notin p[i]} w_{ik}^{(B)} \left( \mathbf{\beta}^{(B)} \cdot \mathbf{x}_{ik}^{(B)} + u_k^{(B)} \right) \tag{18}
\end{equation}

where superscripts $(A)$ and $(B)$ denote the two subsets ($k \in p[i]$ and $k \notin p[i]$, respectively). This allows both subsets to have different structural effects and different aggregation mechanisms.

In `bml`, this is specified using two `mm()` blocks with different mmid variables — one for subset $A$ and one for subset $B$:

```r
mm(id = id(pid_A, gid), vars = vars(x1, x2), w = w(~ 1/n), fn = fn("sum"), RE = TRUE) +
mm(id = id(pid_B, gid), vars = vars(x1, x2), w = w(~ 1/n), fn = fn("sum"), RE = TRUE)
```

**Data format:** Since the two `mm()` blocks use different member ID variables (`pid_A` and `pid_B`), the data should be in a stacked long format where each member gets its own row, with `NA` in the mmid column it does not belong to:

```
gid  pid_A  pid_B  x1    x2    dur_wkb  event_wkb
...
**Example (Coalition Governments):** Subset $A$ is the coalition parties ($k \in p[i]$) and subset $B$ is the opposition parties ($k \notin p[i]$). Government parties have `pid_A` filled and `pid_B = NA`; opposition parties have `pid_B` filled and `pid_A = NA`. Group-level variables (e.g., `dur_wkb`, `event_wkb`) are repeated across all rows within a group, as in the standard long format.

## 10. Comparison with Conventional MMMM
