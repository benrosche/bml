...
- [`mm()`](https://benrosche.github.io/bml/reference/mm.md):
  ...
  - `fn = fn(w ~ 1/n, c = TRUE)`: Weight function (equal weights,
...
influence, we can test whether seat share affects a party’s weight in
the coalition:
...
- The weight function now includes `pseat` (party seat share) with
  parameter `b1`
...
- If `b1 ≈ 0`: Seat share doesn’t affect weights (equal influence)
...
- Explore
  ...
  spatial, network, and aggregation applications
