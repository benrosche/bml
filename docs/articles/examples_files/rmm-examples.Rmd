...
mod.rmm3 <-
  rmm(
    ...
    priors=c("b.w~dnorm(0,1)"), n.iter = 1000, n.burnin = 100, seed=1, monitor = TRUE,
    data = boston_df2
  )
...
data.frame(mod.rmm3$w, sum=rowSums(mod.rmm3$w, na.rm = TRUE)) %>% head()

rowSums(mod.rmm3$w, na.rm = TRUE) %>% sum()
...
mod.rmm4 <-
  rmm(
    ...
    n.iter = 1000, n.burnin = 100, seed=1, monitor = TRUE, priors = c("b.w~dunif(0,1)"),
    data = schoolnets
  )
